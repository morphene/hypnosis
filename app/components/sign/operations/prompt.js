...
const opTemplates = {
  ...
  transfer_to_vesting: {
    color: 'red',
    message: (op) => `Send ${op.to} ${op.amount} as VESTS.`
  },
  withdraw_vesting: {
    color: 'orange',
    ...
    if(ops.length === 1) {
      ...
      switch(ops[0][0]) {
        case 'delegate_vesting_shares':
          return [
            <Header attached='top' key='op-header'><Icon name='power' />Delegate VESTS</Header>,
            <OperationsPromptDelegation account={this.props.account} opData={opData} morphene={this.props.morphene} />
          ]
        case 'transfer':
          return [
