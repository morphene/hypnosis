...
export default class OperationsPromptDelegation extends Component {
  ...
  render() {
    ...
    const totalVestsMorph = parseFloat(this.props.morphene.props.total_vesting_fund_morph.split(" ")[0])
    const totalVests = parseFloat(this.props.morphene.props.total_vesting_shares.split(" ")[0])
    const sp = (vests > 0) ? (totalVestsMorph * vests / totalVests).toFixed(3) : 0
      ...
      if(existingDelegation > 0) {
        existingDelegationWarning = (
          <Grid.Row columns={1}>
            <Grid.Column>
              <Message info>
                A delegation of <strong>{existingDelegation} VESTS</strong> is already active from <AccountName name={opData.delegator}/> to <AccountName name={opData.delegatee}/>. This new delegation will <strong>overwrite</strong> the existing delegation (not add to it).
              </Message>
            </Grid.Column>
          </Grid.Row>
        )
