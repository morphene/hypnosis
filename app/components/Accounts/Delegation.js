...
export default class AccountsProxy extends Component {
  ...
  handleOnChange = (value) => {
    ...
    const sp = (totalVestsMorph * vests / totalVests).toFixed(3);
  ...
  handleOnChangeInput = (e, props) => {
    ...
    const globalProps = this.props.morpheneJS.props;
    const totalVestsMorph = parseFloat(globalProps.total_vesting_fund_morph.split(" ")[0])
    ...
    if (name === 'vests') {
      ...
      sp = (totalVestsMorph * fixed / totalVests).toFixed(3)
      vests = value
    }
    if (name === 'sp') {
      const fixed = parsed.toFixed(3);
      sp = value
      vests = (fixed / totalVestsMorph * totalVests).toFixed(6)
    }
    this.setState({ vests, sp });
  }
                  ...
                  <Grid.Row>
                    ...
                    <Grid.Column width={4}>
                      <Header textAlign="center" size="large">
                        <Header.Subheader style={{marginTop: '20px'}}>VESTS</Header.Subheader>
