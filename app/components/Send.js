...
export default class Send extends Component {
  ...
  setAmountMaximum = (e: SyntheticEvent) => {
    ...
    const amount = accounts[from]['balance'].split(' ')[0];
          ...
          if (derivedKey === memoPublic) {
            ...
            morpheneJS.api.getAccounts([to], (err, result) => {
              if(result.length > 0) {
                ...
                const memoEncrypted = morpheneJS.memo.encode(memoKey, toMemoPublic, `#${cleaned}`);
  ...
  render() {
    ...
    const availableAmount = accounts[this.state.from]['balance'];
    ...
    if (keys.permissions[this.state.from] && keys.permissions[this.state.from].memo && morpheneJS.auth.isWif(keys.permissions[this.state.from].memo)) {
          ...
          <Grid.Row>
            <Grid.Column width={4}>
              ...
              <Form.Field
                ...
                label="MORPH"
                value="MORPH"
                checked={this.state.symbol === 'MORPH'}
