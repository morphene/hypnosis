...
export default class KeysLogin extends Component {
  ...
  handleConfirmAction = (
    ...
    if (confirmed) {
      ...
      const keys = morpheneJS.auth.getPrivateKeys(account, password, [permission]);
    ...
    if (encryptWallet) {
      ...
      encryptionWarning = (
        <Message
          ...
          content="The wallet password if only used to unlock Hypnosis during transactions. If you lose your password, you will be unable to recover your keys. Please make sure to back up your keys and remember your password."
        />
      );
    }
    let display = (
      ...
      >
        <Grid divided>
          <Grid.Row>
            <Grid.Column width={8}>
              <Form.Input
                label="Morphene Account Name"
                name="account"
                value={account}
                onChange={this.handleChange}
              />
              <Form.Input
                label="Morphene Password"
                type="password"
                name="password"
                value={password}
                onChange={this.handleChange}
