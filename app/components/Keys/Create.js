...
export default class KeysCreate extends Component {
  ...
  checkUsername = _.debounce((e, props) => {
    ...
    morpheneJS.api.getAccounts([name], (err, results) => {
      this.setState({
        checkingUsername: false,
  ...
  generateSeed = () => {
    ...
    let masterKey = morpheneJS.auth.getPrivateKeys(name, seed, ['master']);
    let password = 'P' + masterKey.master;
    const types = ['owner', 'active', 'posting', 'memo'];
    let pub = morpheneJS.auth.generateKeys(name, password, types);
    let prv = morpheneJS.auth.getPrivateKeys(name, password, types);
  ...
  render() {
    ...
    const availableFundingMethods = ['MORPH','VESTS'].map((name) => {
      return name === 'MORPH' ? {
        key:name,
        text:name,
        value:name
    ...
    function setClipboardText(text){
      var id = "hypnosis-clipboard-textarea-hidden-id";
                  ...
                  <Grid.Column>
                    <Segment basic>
                      <Header>
                        ...
                        <Header.Subheader>
                          WRITE THIS DOWN (or save in a password manager). This password will allow you to log into any Morphene related service with full permissions. Use the active private key if you only wish to grant active permissions.
                        </Header.Subheader>
                      </Header>
                      <Segment>
                        {this.state.password}
                ...
                <Grid.Row>
                  <Grid.Column>
                    <Message
                      ...
                      content='Ensure the steps above are completed and the data above has been copied elsewhere. The data displayed here can never be retrieved through Hypnosis again.'
                    />
            ...
            <Grid.Column width={7}>
              <Message negative>
                <Message.Content>
                  ...
                  <List bulleted>
                    <List.Item>MORPH: {delegation.fee.amount}</List.Item>
                    <List.Item>VESTS: {delegation.delegation.amount.toFixed(6)}</List.Item>
                  </List>
                </Message.Content>
              </Message>
              <Header size="small">
              ...
              <List bulleted>
                ...
                <List.Item>Once the account has been created, it will be automatically imported into Hypnosis, and you can use either the <strong>Master Password</strong> or the <strong>Posting Private Key</strong> to log into any Morphene powered website.</List.Item>
              </List>
            </Grid.Column>
            <Grid.Column width={9}>
              <Segment basic>
